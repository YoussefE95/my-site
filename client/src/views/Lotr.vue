    ...
    methods: {
        async search(event) {
            try {
                ...
                const searchResults = await fetch(`http://localhost:8888/lotr/search/${this.type}?phrase=${this.phrase}`);
        ...
        async getDetails(id) {
            try {
                const searchDetails = await fetch(`http://localhost:8888/lotr/search/${this.type}/details`, {
                    method: "POST",
